...
public class FileManagerFragment extends Fragment {
    ...
    private void createShortcutDirectly(File file, Container container) {
        try {
            ...
            // Usamos "\\\\" para que o Java escreva duas barras "\\" no arquivo de texto.
            // Quando o atalho for lido, essas duas barras virarão uma única barra "\".
            ...
            } else if (execPath.startsWith("/storage/emulated/0/")) {
                winPath = "Z:" + execPath.replace("/", "\\\\");
            } else {
                winPath = "Z:" + execPath.replace("/", "\\\\");
            }
            
            // Para o workDir (Path=), usamos a mesma lógica ou uma barra simples se não for ser lido como escape
            if (winPath.contains("\\")) {
                // Remove o nome do executável para pegar a pasta
                // Como winPath tem barras duplas agora, precisamos ter cuidado
                String cleanWinPath = winPath.replace("\\\\", "\\"); 
                workDir = cleanWinPath.substring(0, cleanWinPath.lastIndexOf("\\"));
            } else {
                workDir = "D:\\"; 
            }
            ...
            try (PrintWriter writer = new PrintWriter(new FileWriter(desktopFile))) {
                ...
                // O winPath aqui já tem as barras duplas (ex: D:\\Jogos\\Game.exe)
                // Isso garante que no arquivo final fique correto e legível pelo Wine/App
