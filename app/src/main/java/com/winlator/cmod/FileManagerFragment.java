...
public class FileManagerFragment extends Fragment {
    ...
    private String getSmartDisplayName(File file) {
        ...
        boolean isGeneric = filename.length() < 4;
        if (!isGeneric) {
            String lower = filename.toLowerCase();
            for (String gen : genericNames) {
                if (lower.equals(gen) || lower.startsWith(gen + " ") || lower.equals(gen.replace("-", " "))) {
                    isGeneric = true;
                    break;
                }
            }
    ...
    private String cleanGameName(String filename) {
        ...
        name = name.replaceAll("(?i)\\b(v\\d+|repack|setup|installer|portable|goty|edition|codex|fitgirl|dodi|elamigos|gold|ultimate|remastered|director|cut|application|shipping)\\b", "");
