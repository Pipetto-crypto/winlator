...
public class FileManagerFragment extends Fragment {
    ...
    private Button btDriveSelect; // Alterado de MaterialButton para Button
    ...
    private void navigateToContainerDriveC(Container container) {
        // Ajuste o caminho se o seu Winlator usar pastas separadas por ID
    ...
    private void updateDriveButtonLabel(File dir) {
        ...
        // Método compatível com Button padrão (Drawable Left, Top, Right, Bottom)
        String path = dir.getAbsolutePath();
        if (path.contains("/drive_c")) {
            btDriveSelect.setText("Drive C:");
            btDriveSelect.setCompoundDrawablesWithIntrinsicBounds(R.drawable.icon_wine, 0, 0, 0); 
        } else if (path.startsWith(Environment.getExternalStorageDirectory().getAbsolutePath())) {
            btDriveSelect.setText("Drive D:");
            btDriveSelect.setCompoundDrawablesWithIntrinsicBounds(android.R.drawable.stat_sys_phone_call, 0, 0, 0);
        } else if (path.startsWith("/storage") && !path.contains("emulated")) {
             btDriveSelect.setText("External");
             btDriveSelect.setCompoundDrawablesWithIntrinsicBounds(android.R.drawable.stat_sys_data_bluetooth, 0, 0, 0);
        } else {
            btDriveSelect.setText("System (Z:)");
            btDriveSelect.setCompoundDrawablesWithIntrinsicBounds(android.R.drawable.stat_notify_sdcard, 0, 0, 0);
        }
    }
