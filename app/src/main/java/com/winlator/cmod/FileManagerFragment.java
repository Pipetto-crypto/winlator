...
public class FileManagerFragment extends Fragment {
    ...
    private void createShortcutDirectly(File file, Container container) {
        try {
            ...
            // --- CORREÇÃO DE CRASH (IMPORTANTE) ---
            // Usamos '/' em vez de '\'. O Wine entende, e evita que o Android
            // ache que o caminho todo é um nome de arquivo (o que causava o erro EPERM no log).
            
            if (execPath.startsWith("/storage/emulated/0/Download/")) {
                winPath = "D:" + execPath.replace("/storage/emulated/0/Download", ""); 
            ...
            // Diretório de trabalho (Pasta onde está o executável)
            if (winPath.contains("/")) {
            ...
            // Salva na Área de Trabalho do Container (Onde o Winlator realmente lê)
            ...
            try (PrintWriter writer = new PrintWriter(new FileWriter(desktopFile))) {
                ...
                // Executa usando aspas para proteger espaços, mas com barras normais
                ...
                // --- CORREÇÃO DE ÍCONE ---
                // Usamos o nome do app. Se o Wine extrair o ícone, ele aparecerá.
                // Se não, ficará um ícone genérico até você definir um manualmente.
                writer.println("Icon=" + name);
                
                writer.println("Path=" + workDir);
                
                // ID blindado para garantir leitura
