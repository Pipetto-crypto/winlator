...
public class FileManagerFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        // --- ALTERAÇÃO AQUI: Inicia na pasta Downloads ---
        currentDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS);
        if (!currentDir.exists()) {
            currentDir = Environment.getExternalStorageDirectory(); // Fallback para raiz se não achar Downloads
        }
    ...
    private void showDriveMenu() {
        ...
        // --- ALTERAÇÃO AQUI: Drive D agora leva para Downloads ---
    ...
    private void navigateToContainerDriveC(Container container) {
        ...
        // Verifica se a estrutura do Wine existe
        if (driveC.exists() && driveC.isDirectory() && windowsDir.exists()) {
            ...
            new AlertDialog.Builder(getContext())
                ...
                .setMessage("The Wine system files (Drive C:) are missing or incomplete.\n\n" +
                           "Current status: Container created, but Wine hasn't run yet.\n\n" +
                           "Please RUN this container (start the desktop) at least once to generate the filesystem.")
                .setPositiveButton("OK", null)
                .show();
        }
    }
            ...
            try {
                ...
                if (isCutOperation && !isOperationCancelled) {
                    ...
                    if (srcSize > 0 && srcSize == dstSize && source.canRead()) {
                        ...
                        throw new IOException("Safety Stop: Sizes mismatch or source unreadable.");
                    }
                }
    ...
    private void copyFileSafe(File source, File dest, long totalBytes, AtomicLong totalCopied) throws IOException {
        ...
        try {
            ...
            while ((len = in.read(buffer)) > 0) {
                ...
                long currentTotal = totalCopied.addAndGet(len);
                if (currentTotal % (1024 * 1024) == 0) {
    ...
    private void showFileOptions(File file, View anchor) {
        ...
        if (isExecutable(file)) {
            popup.getMenu().add("Create Shortcut (Winlator)");
        }
