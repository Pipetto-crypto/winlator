...
// --- IMPORTS CORRIGIDOS (Baseado no seu ShortcutsFragment) ---
import com.winlator.cmod.container.Container;
import com.winlator.cmod.container.ContainerManager;
// Não precisamos mais do Shortcut ou ShortcutsManager para criar o arquivo manualmente
...
public class FileManagerFragment extends Fragment {
    ...
    private void loadDirectory(File dir) {
        ...
        // Ordenação: Pastas > Executáveis > Outros
    ...
    // --- NOVA LÓGICA: CRIA O ARQUIVO .DESKTOP MANUALMENTE ---
    private void createShortcutDirectly(File file, Container container) {
        try {
            // 1. Prepara os dados
            String name = file.getName();
            int pos = name.lastIndexOf(".");
            if (pos > 0) name = name.substring(0, pos); // Remove extensão
            
            String execPath = file.getAbsolutePath();
            
            // 2. Localiza a pasta interna de atalhos do Winlator
            File shortcutsDir = new File(getContext().getFilesDir(), "shortcuts");
            if (!shortcutsDir.exists()) shortcutsDir.mkdirs();

            // 3. Cria o arquivo .desktop
            File desktopFile = new File(shortcutsDir, name + ".desktop");
            
            // Se já existe, tenta um nome único (ex: Game (1).desktop)
            int counter = 1;
            ...
            // 4. Escreve o conteúdo no formato padrão do Winlator
            try (PrintWriter writer = new PrintWriter(new FileWriter(desktopFile))) {
                writer.println("[Desktop Entry]");
                writer.println("Name=" + name);
                writer.println("Exec=\"" + execPath + "\""); // Aspas são importantes se tiver espaço
                writer.println("Type=Application");
                writer.println("Terminal=false");
                writer.println("StartupNotify=true");
                writer.println("Icon="); // Sem ícone por padrão
