...
import com.winlator.cmod.core.SteamGameFetcher;
...
public class FileManagerFragment extends Fragment {
    ...
    private void createShortcutDirectly(File file, Container container) {
        try {
            // --- NOME INTELIGENTE (Smart Name) ---
            // Decide se usa o nome do arquivo ou da pasta (ex: speed.exe -> Need For Speed)
            String displayName = getSmartDisplayName(file);
            
            // O caminho real para execução continua intacto
            ...
            // Salva o ícone com o nome "bonito"
            File iconFile = new File(iconsDir, displayName + ".png");

            // --- BUSCA NA STEAM ---
            Toast.makeText(getContext(), "Buscando capa para: " + displayName, Toast.LENGTH_SHORT).show();
            
            SteamGameFetcher.fetchIcon(displayName, iconFile, new SteamGameFetcher.OnImageDownloadedListener() {
                @Override
                public void onSuccess(File imageFile) {
                    if (getActivity() != null) {
                        getActivity().runOnUiThread(() -> 
                            Toast.makeText(getContext(), "Capa aplicada: " + displayName, Toast.LENGTH_SHORT).show()
                        );
                    }
                }
            ...
            try (PrintWriter writer = new PrintWriter(new FileWriter(desktopFile))) {
                ...
                writer.println("Name=" + displayName); // Nome limpo
                // O caminho aqui usa barras normais (/), pois o XServerDisplayActivity já foi corrigido para aceitar isso
                ...
                writer.println("Icon=" + displayName); // Aponta para o ícone baixado da Steam
            ...
            e.printStackTrace();
            Toast.makeText(getContext(), "Erro ao criar atalho: " + e.getMessage(), Toast.LENGTH_LONG).show();
        }
    }

    /**
     * Define o nome inteligente do jogo.
     * Se o executável for genérico (game.exe), usa o nome da pasta pai.
     */
    private String getSmartDisplayName(File file) {
        String filename = cleanGameName(file.getName());
        
        // Lista de nomes genéricos que devem ser ignorados
        ...
        // Se o nome for genérico ou muito curto (ex: a.exe)
        if (filename.length() < 4 || genericNames.contains(filename.toLowerCase())) {
            
            File parent = file.getParentFile();
            if (parent != null) {
                String parentName = cleanGameName(parent.getName());
                
                // Se a pasta pai também for genérica (ex: bin, system), sobe mais um nível
                if (parentName.equalsIgnoreCase("bin") || 
                    parentName.equalsIgnoreCase("bin32") || 
                    parentName.equalsIgnoreCase("bin64") ||
                    parentName.equalsIgnoreCase("system") ||
                    parentName.equalsIgnoreCase("release") ||
                    parentName.equalsIgnoreCase("retail")) {
                ...
                return parentName; // Usa o nome da pasta (ex: Dead or Alive 5)
            }
        }
        
        return filename; // Usa o nome do arquivo se for específico (ex: BioShock)
    }
    ...
    private String cleanGameName(String filename) {
        ...
        // Remove termos de "scene"/releases
        name = name.replaceAll("(?i)\\b(v\\d+|repack|setup|installer|portable|goty|edition|codex|fitgirl|dodi|elamigos|gold|ultimate|remastered|director|cut)\\b", "");
