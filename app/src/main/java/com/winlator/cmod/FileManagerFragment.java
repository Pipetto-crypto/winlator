...
public class FileManagerFragment extends Fragment {
    ...
    private void createShortcutDirectly(File file, Container container) {
        try {
            ...
            // --- LÓGICA DE CAMINHO ---
            // Voltamos a usar as barras invertidas '\' pois o WinHandler precisa delas.
            // Para evitar o crash, formataremos a string do Exec com cuidado.
            
            if (execPath.startsWith("/storage/emulated/0/Download/")) {
                // Mapeia Download para D:\
                // Atenção ao replace: garantimos que não falte a barra inicial se estiver na raiz de Download
                String relativePath = execPath.replace("/storage/emulated/0/Download", "");
                if (!relativePath.startsWith("/")) relativePath = "/" + relativePath;
                winPath = "D:" + relativePath.replace("/", "\\");
            } else {
                // Mapeia resto para Z:\
                winPath = "Z:" + execPath.replace("/", "\\");
            }
            
            // Define diretório de trabalho (Pasta onde está o executável)
            if (winPath.contains("\\")) {
                workDir = winPath.substring(0, winPath.lastIndexOf("\\"));
            } else {
                workDir = "D:\\"; 
            }

            // Pasta de atalhos dentro do container
            ...
            try (PrintWriter writer = new PrintWriter(new FileWriter(desktopFile))) {
                ...
                // --- CORREÇÃO FINAL PARA EXECUÇÃO E CRASH ---
                // 1. Usamos winPath com barras invertidas (para funcionar o jogo).
                // 2. Colocamos aspas duplas em volta do caminho.
                // 3. O Shortcut.java vai ler isso e remover as aspas. 
                //    Se houver crash de log novamente, o problema é que o Winlator não consegue
                //    lidar com '\' no nome do log no Android, mas o jogo PRECISA do '\'.
                writer.println("Exec=env WINEPREFIX=\"/home/xuser/.wine\" wine \"" + winPath + "\"");
