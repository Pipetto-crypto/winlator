    ...
    public static void changeServicesStatus(Container container, String startupSelection) {
        ...
        try (WineRegistryEditor registryEditor = new WineRegistryEditor(systemRegFile)) {
            registryEditor.setCreateKeyIfNotExist(false);
             ...
             ArrayList<String> essentialList = new ArrayList<>();
            for (String s : services) {
                essentialList.add(s.substring(0, s.indexOf(":")));
            }
            for (String service : aggressiveServices) {
                ...
                if (selection == Container.STARTUP_SELECTION_AGGRESSIVE) {
                    disable = true; 
                } else if (selection == Container.STARTUP_SELECTION_ESSENTIAL) {
                    
                    if (essentialList.contains(name)) {
                        disable = true;
                    }
                }
                
                if (name.equals("winebus") || name.equals("winehid")) disable = false;
                
                if (name.equals("MountMgr")) disable = false;
