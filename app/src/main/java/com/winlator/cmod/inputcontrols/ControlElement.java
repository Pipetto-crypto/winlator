...
import com.winlator.cmod.math.Mathf; // IMPORTAÇÃO RECUPERADA
...
public class ControlElement {
    ...
    private Range range;
    private byte orientation = 0; // 0 = Horizontal, 1 = Vertical
    ...
    public void draw(Canvas canvas) {
        ...
        if (type == Type.RANGE_BUTTON) {
            ...
            // Lógica de Orientação (Se for horizontal, inverte W/H)
            if (orientation == 0) { // Horizontal? Depende da implementação, mas o padrão parece Vertical. Vamos manter o bounding box.
                 // Se o código original do range button usava orientation, ele ajustaria aqui. 
                 // Como não tenho o código original do Range, mantive o padrão vertical 
                 // mas adicionei o campo orientation para evitar crash.
            }
        ...
        if (type == Type.STICK || type == Type.TRACKPAD) {
            ...
            if (bindings[0] != Binding.NONE) {
                if (text.isEmpty()) {
                     int idToDraw = (iconId > 0) ? iconId : bindings[0].getIconId();
    ...
    private class Scroller {
        ...
        void handleTouchMove(float x, float y, Rect bounds) {
            ...
            // Correção do erro "cannot find symbol Mathf"
