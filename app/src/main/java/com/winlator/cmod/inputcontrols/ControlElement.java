...
public class ControlElement {
    ...
    public enum Shape {
        ...
        ROUND_RECT, 
        SQUARE,     
        OVAL;       // ADICIONADO: OVAL para corrigir o crash mais recente
    ...
    public boolean containsPoint(float x, float y) {
        if (shape == Shape.OVAL || shape == Shape.CIRCLE) {
             // Detecção de toque elíptica/circular precisa
             float dx = x - boundingBox.centerX();
             float dy = y - boundingBox.centerY();
             float rw = boundingBox.width() / 2.0f;
             float rh = boundingBox.height() / 2.0f;
             return (dx*dx)/(rw*rw) + (dy*dy)/(rh*rh) <= 1.0f;
        }
        // Detecção Retangular (Padrão)
        return x >= boundingBox.left && x < boundingBox.right && y >= boundingBox.top && y < boundingBox.bottom;
    }

    public void draw(Canvas canvas) {
        int snappingSize = inputControlsView.getSnappingSize();
        // Aumentei levemente o fator base para tentar compensar o tamanho "estranho"
        ...
        if (type == Type.RANGE_BUTTON) {
            ...
            // Se for Oval ou Retângulo, podemos querer dimensões não quadradas no futuro
            // Por enquanto mantemos quadrado para compatibilidade, mas o shape desenha dentro
        ...
        if (type == Type.STICK || type == Type.TRACKPAD) {
            ...
            if (shape == Shape.RECT || shape == Shape.SQUARE) {
                ...
                // Implementação do OVAL
                roundRectF.set(boundingBox);
                canvas.drawOval(roundRectF, paint);
