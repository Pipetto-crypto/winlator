...
public class ControlElement {
    ...
    public enum Type {
        ...
        // Adicionado para corrigir erro no ControlsEditorActivity
    ...
    // CORREÇÃO: Retorna byte para satisfazer ControlsEditorActivity (loadIcons espera byte)
    ...
    // CORREÇÃO: Adicionado método setBindingCount que faltava
    ...
    public void draw(Canvas canvas) {
        ...
        if (type == Type.RANGE_BUTTON) {
            ...
            // Ajuste básico de orientação se necessário no futuro
        ...
        if (type == Type.STICK || type == Type.TRACKPAD) {
            ...
            if (bindings[0] != Binding.NONE) {
                if (text.isEmpty()) {
                     // CORREÇÃO: Binding não tem getIconId, usamos ordinal() como fallback
                     // Se iconId > 0 (definido manualmente), usa ele. Senão usa o ordinal do binding.
