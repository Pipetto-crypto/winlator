...
public class ControlElement {
    ...
    public enum Range {
        _100 // Adicionado para compatibilidade, valor padrão
    }
    ...
    public void draw(Canvas canvas) {
        ...
        if (type == Type.STICK || type == Type.TRACKPAD) {
            ...
            if (bindings[0] != Binding.NONE) {
                if (text.isEmpty()) {
                     // Prioridade: IconId próprio > IconId do Binding
    ...
    public boolean handleTouchUp(int pointerId) {
        if (currentPointerId == pointerId) {
            // Lógica Toggle: não solta o botão no TouchUp
            if (type == Type.BUTTON && toggleSwitch) {
                currentPointerId = -1;
                return true;
            }
    ...
    private boolean isEngaged() {
        if (type == Type.BUTTON || type == Type.RANGE_BUTTON) {
            if (toggleSwitch && type == Type.BUTTON) return states[0]; // Se for toggle, verifica estado
